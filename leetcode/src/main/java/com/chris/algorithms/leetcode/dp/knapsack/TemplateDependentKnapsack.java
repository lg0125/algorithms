...
/** 有依赖的背包(模版)
 ...
 测试链接 : <a href="https://www.luogu.com.cn/problem/P1064">...</a>
 测试链接 : <a href="https://www.nowcoder.com/practice/f9c6f980eeec43ef85be20755ddbeaf4">...</a>
 */
public class TemplateDependentKnapsack {
    public static class Solution {
        public static int MAX_M = 61;

        public static int[] cost = new int[MAX_M];
        public static int[] val = new int[MAX_M];
        public static boolean[] king = new boolean[MAX_M];
        public static int[] fans = new int[MAX_M];
        public static int[][] follows = new int[MAX_M][2];
                ...
                if (king[i]) {
                    for (int j = 0; j <= n; j++) {
                        ...
                        if (fan1 != -1 && j - cost[i] - cost[fan1] >= 0)
                            ...
                            dp[i][j] = Math.max(
                                    dp[i][j],
                                    dp[p][j - cost[i] - cost[fan1]] + val[i] + val[fan1]
                            );
                        if (fan2 != -1 && j - cost[i] - cost[fan2] >= 0)
                            // 可能性4 : 主 + 附2
                            dp[i][j] = Math.max(
                                    dp[i][j],
                                    dp[p][j - cost[i] - cost[fan2]] + val[i] + val[fan2]
                            );
                ...
                if (king[i]) {
                    for (int j = n; j >= cost[i]; j--) {
                        ...
                        if (fan1 != -1 && j - cost[i] - cost[fan1] >= 0)
                            dp[j] = Math.max(
                                    dp[j],
                                    dp[j - cost[i] - cost[fan1]] + val[i] + val[fan1]
                            );
                        if (fan2 != -1 && j - cost[i] - cost[fan2] >= 0)
                            dp[j] = Math.max(
                                    dp[j],
                                    dp[j - cost[i] - cost[fan2]] + val[i] + val[fan2]
                            );
                        if (fan1 != -1 && fan2 != -1 && j - cost[i] - cost[fan1] - cost[fan2] >= 0)
                            dp[j] = Math.max(
                                    dp[j],
