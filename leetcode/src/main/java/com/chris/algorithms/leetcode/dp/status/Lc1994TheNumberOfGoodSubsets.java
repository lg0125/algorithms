package com.chris.algorithms.leetcode.dp.status;
...
/** 好子集的数目
 ...
 测试链接 : <a href="https://leetcode.cn/problems/the-number-of-good-subsets/">...</a>
 */
public class Lc1994TheNumberOfGoodSubsets {
    public static class Solution {
        public static int MAX_V = 30;
        ...
        public static int numberOfGoodSubsets1(int[] nums) {
            // 1 ~ 30
            int[] cnt = new int[MAX_V + 1];
            for (int num : nums)
                cnt[num]++;

            int[][] dp = new int[MAX_V + 1][LIMIT];
            for (int i = 0; i <= MAX_V; i++)
                Arrays.fill(dp[i], -1);

            int ans = 0;
            for (int s = 1; s < LIMIT; s++)
                ans = (ans + f1(MAX_V, s, cnt, dp)) % MOD;
            return ans;
        }
