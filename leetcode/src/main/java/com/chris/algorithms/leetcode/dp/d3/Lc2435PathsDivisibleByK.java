...
/** 矩阵中和能被 K 整除的路径
 ...
 测试链接 : <a href="https://leetcode.cn/problems/paths-in-matrix-whose-sum-is-divisible-by-k/">...</a>
 */
public class Lc2435PathsDivisibleByK {
    public static class Solution {
        ...
        private static int f2(int[][] grid, int n, int m, int k, int i, int j, int r, int[][][] dp) {
            if (i == n - 1 && j == m - 1)
                return grid[i][j] % k == r ? 1 : 0;
            if (dp[i][j][r] != -1)
                return dp[i][j][r];

            int need = (k + r - grid[i][j] % k) % k, ans = 0;
